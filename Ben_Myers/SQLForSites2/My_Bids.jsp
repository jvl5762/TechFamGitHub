...
<%while(result_auction != null && result_auction.next()){
	...
	int bid_id = result_auction.getInt("bid_id");
	...
	long cancel_long = result_auction.getLong("cancellation_timestamp");
	...
	Float your_bid = result_auction.getFloat("your_amount");
	boolean is_auto = result_auction.getBoolean("is_auto");
	String auto = "Manual";
...
<tr>
  ...
  <tr>
    ...
    <td><%= auto%></td>
    <%if(is_auto){ %>
    <th><%= result_auction.getFloat("max_amount")%></th>
    <%} %>
  </tr>
</table>
<%if(!is_auto){ %>
...
<input type="hidden" name="auction_timestamp_start" value=<%= start_long%> />
<input type="hidden" name="bid_id" value=<%= bid_id%> />
<input button value="Cancel" type="submit"/>
</form>
<% }%>
</div>
 <%} %>
