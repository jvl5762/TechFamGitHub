...
Connection con = null;
PreparedStatement select_highest_bid, select_reserved_price, delete_auction, add_to_sale, select_bid_info, select_sale_id;
ResultSet result_highest_bid, result_reserved_price, result_bid_info, result_max_id;
...
try{
	...
	if(highest_bid >= reserved_price){
		// get bid info
		select_bid_info = con.prepareStatement("SELECT username, credit_card_number, address_id FROM Bid " +
		...
		username = result_bid_info.getString(1);
		
		credit_card_number = result_bid_info.getLong(2);
		
		address_id = result_bid_info.getInt(3);
		...
		add_to_sale.setLong(3, System.currentTimeMillis());
		
		add_to_sale.setBoolean(4, IDENTIFY_AUCTION);
		
		add_to_sale.setInt(5, item_id);
		
		add_to_sale.setString(6, username);
		
		add_to_sale.setLong(7, credit_card_number);
		
		add_to_sale.setInt(8, address_id);
